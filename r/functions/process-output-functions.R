...
process_HI = function(.dt, .scen, .time) {
  if (.scen %like% 'cc') { # get agcacc
    # select relevant columns
    cols    = c('gridid', 'crop', 'scenario', 'irr', 'ssp', 'gcm', 'time', 'crpval', 'hi', 'cgrain', 'agcacc')
    file1   = .dt[scenario == .scen, ..cols]
    gc()
  } else {
    # select relevant columns
    cols    = c('gridid', 'crop', 'scenario', 'irr', 'ssp', 'gcm', 'time', 'hi', 'cgrain')
    file1   = .dt[scenario == .scen, ..cols]
    gc()
  }
  ...
  if (.scen %like% 'cc') { # get agcacc
    # select relevant columns
    file1[, run_yrs := sum(run_yrs), by = .(gridid, crop, scenario, irr, ssp, gcm, crpval)]
  } else {
    file1[, run_yrs := sum(run_yrs), by = .(gridid, crop, scenario, irr, ssp, gcm)]
  }
  gc()
