...
#                 - Unzips tar files
#                 - Creates separate dt for harvest index & crop yield subset by simulation years
#-----------------------------------------------------------------------------------------
# LIBRARIES
#-----------------------------------------------------------------------------------------
library(data.table)
...
if (length(args) != 3) {
  stop(
    ...
    "  3) scenario\n",
    "Example: Rscript r/process-output-hi.R dec-23 2036 conv"
  )
}
...
main.dir  = getwd() 
print(main.dir)
working.dir = file.path(main.dir, "data", "model-output", args[1]) 
rda.dir     = file.path(main.dir, "data", "model-output", "harvest-index","output")
...
# STEP 1.2.1 HI
print('Running HI function.')
dt_HI = process_HI(file, scenarios, s_yr)

# SAVE
fname_HI     = paste(dt_HI[1, ssp], scenarios,'HI-results.csv', sep = '-')
print(paste0('Saving HI raw results file for ', dir))
fwrite(dt_HI, file = paste(rda.dir, fname_HI, sep = '/'), append = TRUE)
