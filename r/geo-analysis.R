...
# Visualizations
#-------------------------------------------------------------------------------
#ok let me start very small and filter this data down to just the US
#I want to try my hand at a map that just shows d_s_SOC in the USA
dt_small <- dt_scenario[WB_NAME == "United States of America", ]

ggplot(dt_small, aes(x = x, y = y, fill = d_s_SOC)) +
  geom_raster() +
  
  scale_fill_distiller(palette = "RdYlGn", direction = 1,
                       name = "SOC Change\n(Mg C/ha)") +
  coord_sf() +
  labs(title = paste("Delta Cumulative SOC |", dt_small$WB_NAME[1]),
